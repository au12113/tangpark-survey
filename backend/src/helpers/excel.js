  ...
  return await jsonSheet.reduce((prev, row, rowIndex) => {
    Object.keys(row).forEach(async (key) => {
      ...
      if (dateColumn.indexOf(key) !== -1) {
        ...
        } else if (__argv['mode'] === 'potential' && key === 'ลักษณะลูกค้า') {
          if (row[key] === 'ไม่มีชื่อบริษัท เช่น เป็นกลุ่มวิ่งร่วม หรือกลุ่มถมดิน') {
            row['ชื่อบริษัท(ลูกค้า)'] = row['ชื่อกลุ่ม และ/หรือหัวหน้ากลุ่ม']
            delete row['ชื่อกลุ่ม และ/หรือหัวหน้ากลุ่ม']
          } else {
            row['ชื่อบริษัท(ลูกค้า)'] = row['คำนำหน้าบริษัท'] + ' ' + row['ชื่อบริษัท']
            let removeKey = [key, 'คำนำหน้าบริษัท', 'ชื่อบริษัท']
            removeKey.forEach((e) => {
              delete row[e]
            })
          }
...
const addImageToExcel = async (filePath, ws_column, imgCol = [], savePath = './output/excel/output.xlsx') => {
  ...
  let ws2 = wb.addWorksheet('Sheet2', { properties: { defaultColWidth: 20 }})
  return new Promise(async (resolve, reject) => {
    try {
      ws.eachRow(async (row, rowNumber) => {
        if (rowNumber !== 1) {
          let tmp = row.values.slice(0, 4).concat(['ที่ปรึกษาการขาย'], row.values.slice(4, -1))
          ws2.addRow(tmp)
          ...
          ws2.addRow(['ผู้จำหน่าย', 'วันที่เข้าพบลูกค้า', 'ชื่อผู้ที่เข้าเยี่ยม', 'ตำแหน่ง', 'ชื่อบริษัท(ลูกค้า)', 'ชื่อ - นามสกุล(ลูกค้า)', 'ลักษณะธุรกิจ', 'สินค้าที่บรรทุก', 'จำนวนรถในครอบครอง', 'กลุ่มเป้าหมาย', 'แนบรูปภาพ'])
        }
      })
    } catch (e) {
      reject(e)
...
const writeExcel = async (jsonSheet, filename, subFolder=undefined) => {
  ...
  if (__argv['mode'] == 'mapping') {
    ...
    ws_column = ["สาขาที่ออกเยี่ยม", "วันที่ออกเยี่ยม", "ชื่อscที่ออกเยี่ยม ", "ชื่อบริษัท(ลูกค้า)", "ชื่อลูกค้า ", "ประเภทธุรกิจ", "ประเภทสินค้าที่ขนส่ง เช่น ยางพารา",
    "คำอธิบายรถ ISUZU", "ประเภทธุรกิจ", "อัพโหลดรูปออกเยี่ยม"]
  }
  ...
  if (__argv['mode'] === 'potential') {
    await addImageToExcel(tmpPath, ws_column, ["อัพโหลดรูปออกเยี่ยม"], savePath)
  }
}
